...
function Lobby({ session, table, setTable, start, setReady, signOut, tell, lobbyRefreshVersion }) {
  ...
  const me = table.players.find((player) => player.playerId === session.player.playerId);
  const allReady = table.players.length >= 2 && table.players.every((player) => player.ready);
  const waitingFor = table.players.filter((player) => !player.ready).length;
  const waitingMessage = waitingFor === 0 ? "" : waitingFor === table.players.length ? "Waiting for all players to be ready." : `Waiting for ${waitingFor} player${waitingFor === 1 ? "" : "s"} to be ready.`;
  return <section className="panel table-lobby"><div className="table-heading"><div><button className="text-button" type="button" onClick={() => setTable(null)}>← Back to lobby</button><p className="eyebrow">Table #{table.tableId} - starting bet {table.entryBet}</p><h2>{table.tableName}</h2></div><span className={`status ${table.status.toLowerCase()}`}>{table.status}</span></div><p className="muted">Share this table ID. Each new player receives 10,000 virtual coins.</p><div className="seat-list">{table.players.map((p) => <div className="seat" key={p.playerId}><span className="seat-number">{p.seatNumber}</span>{p.username}{p.playerId === table.hostPlayerId && <em>Host</em>}<em>{p.ready ? "Ready" : "Not ready"}</em></div>)}</div>{waitingMessage && <p className="muted">{waitingMessage}</p>}<div className="actions">{me && <button className="secondary" disabled={busy} onClick={() => setReady(!me.ready)}>{me.ready ? "Cancel" : "Ready"}</button>}{host && allReady && <button className="primary" disabled={busy} onClick={start}>Deal cards</button>}<button className="secondary" onClick={async () => { try { setTable(await api(`/tables/${table.tableId}`, session.credentials)); } catch (e) { tell(e.message, true); } }}>Refresh</button><button className="text-button" type="button" onClick={confirmSignOut}>Sign out</button></div></section>;
}
...
function BettingControls({ round, canSame, canShow, canSideShow, confirmLeave, run }) {
  ...
  return <div className="betting-controls">
    ...
    <div className="betting-controls-bottom">
      ...
      <button className="secondary" disabled={!canSideShow} onClick={() => run(`/rounds/${round.roundId}/side-shows`, { amount })}>Side Show</button>
    </div>
  </div>;
}

function Table({ session, table, round, hand, showdown, live, run, leaveRound, returnToLobby }) {
  const mine = round.players?.find((p) => p.playerId === session.player.playerId); const myTurn = round.currentTurnPlayerId === session.player.playerId; const canSame = round.phase === "FINAL_TWO" || (mine?.sameBetActions ?? 0) < 5; const activePlayerCount = (round.players || []).filter((player) => player.status === "ACTIVE" && player.live !== false).length; const canShow = activePlayerCount === 2; const canSideShow = activePlayerCount >= 2;
  ...
  return <section className="game-board"><header className="game-header"><div><button className="text-button" type="button" onClick={confirmLeave}>← Back / Leave game</button><p className="eyebrow">Round {round.roundNumber} - pot {round.pot} - bet {round.currentBet}</p><h2>{table.tableName}</h2></div><div className="live-status"><span className={live ? "live-dot connected" : "live-dot"} />{live ? "LIVE" : "RECONNECTING"}</div></header>
    ...
    {!showdown && myTurn && ["BETTING", "FINAL_TWO"].includes(round.phase) && <BettingControls key={`${round.roundId}:${round.currentBet}`} round={round} canSame={canSame} canShow={canShow} canSideShow={canSideShow} confirmLeave={confirmLeave} run={run} />}
    ...
    {showdown && <div className="round-message">{showdown.winnerUsername} wins: {showdown.winningHandCategory.replaceAll("_", " ")}</div>}{showdown && <div className="actions centered"><button className="primary" onClick={returnToLobby}>Back to waiting room</button></div>}
  </section>;
}
...
export default function Game() {
  ...
  async function setReady(ready) { try { setTable(await api(`/tables/${table.tableId}/play-again`, session.credentials, "POST", { ready })); } catch (e) { tell(e.message, true); } }
  const isAdmin = session?.player?.role === "ADMIN";
  return <main className="app-shell"><header className="brand">ACEVERSE <small>Authoritative Teen Patti</small>{session && <span className="identity">{session.player.username} - {session.player.coinBalance} coins</span>}{isAdmin && <button className="text-button" type="button" onClick={openAdmin}>Admin dashboard</button>}<button className="text-button sound-toggle" type="button" onClick={toggleSound}>{soundEnabled ? "🔊 Sound ON" : "🔇 Sound OFF"}</button></header><Message error={error} notice={notice} />{!session ? <Auth setSession={setSession} tell={tell} /> : adminView && isAdmin ? <AdminDashboard session={session} tell={tell} close={closeAdmin} refreshVersion={adminRefreshVersion} /> : !round ? <Lobby session={session} table={table} setTable={setTable} start={start} setReady={setReady} signOut={signOut} tell={tell} lobbyRefreshVersion={lobbyRefreshVersion} /> : <Table session={session} table={table} round={round} hand={hand} showdown={showdown} live={live} run={run} leaveRound={() => run(`/rounds/${round.roundId}/leave`)} returnToLobby={() => { setRound(null); setHand(null); setShowdown(null); }} />}</main>;
}
